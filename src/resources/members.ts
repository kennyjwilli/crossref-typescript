...
export class Members extends APIResource {
   ...
   */
  retrieve(id: number, options?: RequestOptions): APIPromise<MemberRetrieveResponse> {
   ...
   * - `prefix:10.2481`
   *
   * would locate members with no current DOIs and no backfile DOIs and with prefix
   * 10.1296 or 10.2481. These filters would be specified by joining each filter
   ...
   * ##
   *
   * - `backfile-doi-count` - members with given count of DOIs for material published
   *   more than two years ago
   * - `current-doi-count` - members with given count of DOIs for material published
  ...
  /**
   * Returns list of works associated with a Crossref member (deposited by a Crossref
   * member) with {id}.
   *
   * ## Queries
   *
  ...
  retrieveWorks(
    id: number,
    query: MemberRetrieveWorksParams | null | undefined = {},
    options?: RequestOptions,
  ): APIPromise<WorksMessage> {
    return this._client.get(path`/members/${id}/works`, { query, ...options });
  }
}

export interface BreakdownsObject {
...
export namespace WorksMessage {
  export interface Message {
    items: Array<WorksAPI.Work>;
...
export interface MemberListResponse {
  message: MemberListResponse.Message;
...
export interface MemberRetrieveWorksParams {
  /**
   * Exposes the ability to deep page through large result sets, where offset would
   * cause performance problems
   */
