...
export class Prefixes extends APIResource {
  ...
  listWorks(
    prefix: string,
    query: PrefixListWorksParams | null | undefined = {},
    options?: RequestOptions,
  ): APIPromise<MembersAPI.WorksMessage> {
    return this._client.get(path`/prefixes/${prefix}/works`, { query, ...options });
  }

  /**
   * Returns metadata for the DOI owner prefix, as an example use prefix 10.1016
   */
  retrieveMetadata(prefix: string, options?: RequestOptions): APIPromise<PrefixRetrieveMetadataResponse> {
    return this._client.get(path`/prefixes/${prefix}`, options);
  }
}

export interface PrefixRetrieveMetadataResponse {
  message: PrefixRetrieveMetadataResponse.Message;
...
export namespace PrefixRetrieveMetadataResponse {
...
export interface PrefixListWorksParams {
  /**
