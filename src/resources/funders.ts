...
export class Funders extends APIResource {
  /**
   * Returns metadata for specified funder **and** its suborganizations, as an
   * example use id 501100006004
   */
  retrieve(id: string, options?: RequestOptions): APIPromise<FunderRetrieveResponse> {
    return this._client.get(path`/funders/${id}`, options);
  }

  /**
   * Returns a list of all funders in the
   * [Funder Registry](https://gitlab.com/crossref/open_funder_registry).
   *
   * ## Queries
   *
   ...
   *
   * Offsets can be used to iterate over the results. For this route, the maximum
   * number of available results is 80,000, which in this case allows to retrieve all
   * the indexed items. This limit applies to the sum of values of parameters
   * `offset` + `rows`.
   *
   * ##
   *
   ...
   */
  list(
    query: FunderListParams | null | undefined = {},
    options?: RequestOptions,
  ): APIPromise<FunderListResponse> {
    return this._client.get('/funders', { query, ...options });
  }
  ...
  listWorks(
    id: string,
    query: FunderListWorksParams | null | undefined = {},
    options?: RequestOptions,
  ): APIPromise<MembersAPI.WorksMessage> {
    return this._client.get(path`/funders/${id}/works`, { query, ...options });
  }
}

export interface FunderRetrieveResponse {
  message: FunderRetrieveResponse.Message;
...
export namespace FunderRetrieveResponse {
  export interface Message {
    ...
    'hierarchy-names': { [key: string]: string };
...
export interface FunderListResponse {
  message: FunderListResponse.Message;
...
export namespace FunderListResponse {
  export interface Message {
    items: Array<Message.Item>;

    'items-per-page': number;

    query: MembersAPI.Query;
...
export interface FunderListWorksParams {
  /**
   * Exposes the ability to deep page through large result sets, where offset would
   * cause performance problems
   */
...
export declare namespace Funders {
  export {
